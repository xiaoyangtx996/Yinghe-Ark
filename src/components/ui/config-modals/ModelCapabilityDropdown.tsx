...
export function ModelCapabilityDropdown({
    ...
    const textSize = compact ? 'text-[length:var(--glass-font-size-caption)]' : 'text-[length:var(--glass-font-size-body)]'
    const modelOptionTextSize = compact ? 'text-[length:var(--glass-font-size-caption)]' : 'text-[length:var(--glass-font-size-body)]'
    ...
    return (
        <div ref={triggerRef}>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                        {selectedModel && (paramSummary || selectedModel.providerName || selectedModel.provider) && (
                            <span className="relative group/info">
                                ...
                                <span className="pointer-events-none absolute right-0 bottom-full z-[60] mb-2 whitespace-nowrap rounded-lg glass-surface-modal px-3 py-1.5 text-[length:var(--glass-font-size-caption)] text-[var(--glass-text-primary)] opacity-0 transition-opacity group-hover/info:opacity-100">
            ...
            {isOpen && createPortal(
                <div
                    ref={panelRef}
                    className="glass-surface-modal z-[9999] overflow-hidden flex flex-col rounded-[var(--glass-radius-lg)] shadow-[var(--glass-shadow-2)]"
                    style={panelStyle}
                >
                    {/* Model list */}
                            ...
                            return Array.from(grouped.entries()).map(([providerLabel, groupModels]) => (
                                <div key={providerLabel} className="mb-1">
                                    <div className="sticky top-0 z-10 bg-[var(--glass-bg-surface-strong)] px-2 pt-2 pb-1 backdrop-blur-md">
                                        <span className="text-[length:var(--glass-font-size-caption)] font-medium text-[var(--glass-text-tertiary)] tracking-wide">
                                            {providerLabel}
                                        </span>
                                    </div>
                                        ...
                                        {groupModels.map((m) => (
                                            <button
                                                ...
                                                className={`w-full text-left px-4 py-2 transition-all border-l-[3px] ${value === m.value
                                                    ? 'border-[var(--glass-tone-info-fg)] bg-[var(--glass-bg-surface-strong)] font-medium'
                                                    : m.disabled
                                                        ? 'border-transparent text-[var(--glass-text-disabled)] cursor-not-allowed'
                                                        : 'border-transparent hover:bg-[var(--glass-bg-hover)]'
                                                    }`}
                                            >
                                                <span className={value === m.value
                                                    ? `${modelOptionTextSize} font-medium text-[var(--glass-text-primary)]`
                    ...
                    {(visibleCapabilityFields.length > 0 || booleanToggles.length > 0) && (
                        <div data-capability-params className="shrink-0 bg-[var(--glass-bg-surface)]">
                            <div className="px-4 py-3">
                                <div className="text-[length:var(--glass-font-size-caption)] font-medium text-[var(--glass-text-secondary)] uppercase tracking-wider mb-2.5">
                                    {t('paramConfig')}
                                </div>
                                        ...
                                        {visibleCapabilityFields.map((def) => {
                                            ...
                                            return (
                                                <div key={def.field} className="flex items-center justify-between gap-3">
                                                    <span className="text-[length:var(--glass-font-size-body)] text-[var(--glass-text-secondary)] font-semibold shrink-0">
                                                        {resolveCapabilityLabel(def)}
                                                    </span>
                                                    {def.options.length === 1 ? (
                                                        <span className="text-[length:var(--glass-font-size-caption)] font-medium px-2.5 py-1 rounded-md bg-[var(--glass-bg-surface-strong)] border border-[var(--glass-stroke-base)] text-[var(--glass-text-secondary)] flex items-center gap-1">
                                                            {(() => {
                                                            ...
                                                            <span className="text-[var(--glass-text-tertiary)] text-[length:var(--glass-font-size-caption)]">({t('fixed')})</span>
                                                        </span>
                                                    ) : useSelect ? (
                                                        <div className="relative group">
                                                            <select
                                                                ...
                                                                className="appearance-none bg-transparent hover:bg-[var(--glass-fill-hover)] text-[length:var(--glass-font-size-body)] font-medium text-[var(--glass-text-primary)] pl-3 pr-7 py-1 rounded-md transition-colors outline-none cursor-pointer border border-transparent"
                                                            >
                                                                {def.options.map((opt) => {
                                                                    const s = String(opt)
                                                                    return (
                                                        ...
                                                        <div className="flex bg-[var(--glass-bg-muted)] p-[3px] rounded-lg shadow-inner">
                                                            {def.options.map((opt) => {
                                                                const s = String(opt)
                                                                ...
                                                                return (
                                                                    <button
                                                                        ...
                                                                        className={`px-3 py-1.5 text-[length:var(--glass-font-size-caption)] font-medium rounded-md transition-all flex items-center gap-1 cursor-pointer ${on
                                                                            ? 'bg-[var(--glass-bg-surface-strong)] text-[var(--glass-text-primary)] shadow-[var(--glass-shadow-sm)] font-medium border border-[var(--glass-stroke-base)]'
                                                                            : disabled
                                                                                ? 'text-[var(--glass-text-disabled)] cursor-not-allowed'
                                            ...
                                            <div key={toggle.key} className="flex items-center justify-between gap-3">
                                                ...
                                                <div className="flex bg-[var(--glass-bg-muted)] p-[3px] rounded-lg shadow-inner">
                                                    <button
                                                        ...
                                                        className={`px-3 py-1.5 text-[length:var(--glass-font-size-caption)] font-medium rounded-md transition-all ${toggle.value
                                                            ? 'bg-[var(--glass-bg-surface-strong)] text-[var(--glass-text-primary)] shadow-[var(--glass-shadow-sm)] font-medium border border-[var(--glass-stroke-base)]'
                                                            : 'text-[var(--glass-text-tertiary)] hover:text-[var(--glass-text-secondary)]'
                                                            }`}
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className={`px-3 py-1.5 text-[length:var(--glass-font-size-caption)] font-medium rounded-md transition-all ${!toggle.value
                                                            ? 'bg-[var(--glass-bg-surface-strong)] text-[var(--glass-text-primary)] shadow-[var(--glass-shadow-sm)] font-medium border border-[var(--glass-stroke-base)]'
                                                            : 'text-[var(--glass-text-tertiary)] hover:text-[var(--glass-text-primary)] dark:hover:text-[var(--glass-text-primary)]'
                                                            }`}
                                                    >
                                                        {toggle.offLabel || 'Off'}
                                                    </button>
